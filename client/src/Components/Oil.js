...
const Oil = ({ searchQuery }) => {
  const { oil } = UseAuth();
  const dispatch = useDispatch();

  // Check if oil is an array before calling map function
  if (!Array.isArray(oil)) {
    console.error("oil data is not an array:", oil);
    return null; // Render nothing if oil is not an array
  }
  const filterProduct = oil.filter((oil) =>
    oil.title.toLowerCase().includes(searchQuery.toLowerCase())
  );
